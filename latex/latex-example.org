...
* References
  ...
  *TODO:* The sorting does not yet seem to get correctly observed.

  The =#+BIBLIOGRAPHY:= command inserts the reference list. It requires the
  name of the bib-file (without .bib extension), the name of a style (e.g. plain)
  and one can pass some options (look at the comments section of =ox-bibtex.el=)
  ...
  To get the citations correctly rendered, one needs to add a bibtex invocation
  to the latex command chain:
