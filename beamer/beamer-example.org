...
** addtional LaTeX packages
   ...
   # xcolor which seems to get triggered by pgf or so
...
** Text colors
   ...
   TODO: Currently, only the xcolor default names are available,
   because the xcolor gets pulled in without options by an
   early dependency. Need to investigate.
   
* COMMENT Animations by overlays
** Highlighting text
