...
*** data frame printing using Ipython.display   
   ...
   to align a frame, but it does not provide the same quality as the
   above solution.
   ...
   :RESULTS:
	   A  B    C         D         E
   0     one  A  foo  1.184277  0.989279
   1     one  B  foo  1.151020  0.639264
   2     two  C  foo -0.446286 -1.520620
   3   three  A  bar -0.457811  1.765751
   4     one  B  bar  0.293829 -1.321348
   5     one  C  bar -1.431481 -1.290106
   6     two  A  foo -0.189017 -0.725013
   7   three  B  foo -1.198120  0.380463
   8     one  C  foo -0.818436  0.710930
   9     one  A  bar -0.967236 -1.017655
   10    two  B  bar  0.520839 -0.453348
   11  three  C  bar -1.631013 -1.132096
   :END:
...
* Sympy
  ...
  The correct preview of the resulting LaTeX fragment I get with the
  /output drawer/ result options. I tested rendering with the
  =:results latex= option, but the resulting LaTeX block is not
  rendered by the =org-toggle-latex-fragment= command.
  ...
  For simple in-buffer consumation, one may also want to just use the ASCII output
