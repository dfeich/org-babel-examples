...
* Pandas
** Reading in a data frame with the column names
   #+BEGIN_SRC python :tangle orgbabelhelper/orgbabelhelper/orgbabelhelper.py
     # NOTANGLE-START
     import pandas as pd
     # NOTANGLE-END

     def orgtable_to_dataframe(tbl, index=None):
         """Read an org table into a data frame. Make sure you use ':colnames no'
         in your src block headers. Else the first row containing the column names
         will not be available to the python code."""
         df = pd.DataFrame(tbl)
   ...
   Here, I am importing the orgbabelhelper package that is built from the tangled
   code blocks.
   
   #+BEGIN_SRC python :results output :var tbl=tableA :colnames no
     import orgbabelhelper as obh
     print obh.orgtable_to_dataframe(tbl)
     print
     print obh.orgtable_to_dataframe(tbl,'name')
     print
     print obh.orgtable_to_dataframe(tbl,1)
...
#+NAME: dframeToOrg
  ...
  def dataframe_to_orgtable(dframe, name=None, caption=None, attr=None,
   ...
   Naturally, I could also have used the =import orgbabelhelper as obh= as above.
